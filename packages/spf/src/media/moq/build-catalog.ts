/**
 * MSF catalog building (draft-ietf-moq-msf-01 §5) — the publish-direction complement to `parse-catalog.ts`.
 *
 * Emits the independent-catalog JSON a publisher sends as object 0 of a new group on its catalog track. The output is
 * deliberately the exact dialect `parse-catalog.ts` consumes — every emitted document must round-trip through
 * `applyMoqCatalogUpdate` + `moqCatalogToPresentation` into tracks `codec-mapping.ts` can configure decoders from; that
 * round-trip is the module's acceptance test.
 *
 * One LOC-packaged camera video rendition, an optional second LOC-packaged screen-share video rendition (additive, not
 * an alternate — see the multi-source design record), one LOC-packaged audio track, plus any application data tracks
 * (name + role only — non-renderable, so the parse side classifies them as engine plumbing rather than media). When a
 * screen track is present, all tracks share one `renderGroup` (never `altGroup`, which marks alternates of the same
 * content) so subscribers know camera + screen + audio compose one live view; a camera-only catalog stays exactly as it
 * was before screen share existed. The absent `altGroup` is the load-bearing half of that pair on the parse side: it is
 * what puts camera and screen in separate switching sets, so a subscriber's ABR never mistakes the screen share for a
 * cheaper camera. Codec strings are WebCodecs registry strings (§5.2.18) and pass through verbatim — the caller
 * resolves what to declare (`derive-catalog.ts` re-derives H.264 strings from the published bitstream). A track's
 * decoder init data (`initData`) is emitted as an inline `initDataList` entry plus a per-track `initRef` (§5.2.16-17) —
...
export interface MsfCatalogVideoTrackInput {
  ...
  bitrate?: number;
  /**
   * Decoder init data (WebCodecs `decoderConfig.description`, e.g. avcC for `avc`-format H.264) — emitted as an inline
   * `initDataList` entry referenced by the track's `initRef`.
   */
  initData?: Uint8Array;
}
...
export interface MsfCatalogDataTrackInput {
  name: string;
  /**
   * MSF role label (§5.2.6) classifying the non-media track for consumers, e.g. `'data'` or `'metadata'`. MUST NOT be a
   * media role (`isMediaCatalogRole`): the entry carries no codec or media fields, so a media role would round-trip
   * into an undecodable renderable track. Like codec strings, the caller resolves what to declare — the publish engine
   * strips media roles before building (`resolveDataTracks`).
   */
  role?: string;
}
...
export interface MsfCatalogInput {
  ...
  /**
   * Application data tracks published beside the media — advertised so consumers can discover and subscribe to them,
   * but carrying none of the media fields (`mediaKindOf` on the parse side classifies them as non-renderable
   * plumbing).
   */
...
/**
 * Register a track's init data and return the `initRef` naming it. Ids derive from the track name, which is unique
 * within the catalog.
 */
 ...
 *
 * Namespaces are emitted explicitly per track as the path-joined string form `parse-catalog.ts` splits back into tuple
 * fields — the same (interop-pending) encoding the parser assumes, so the pair stays self-consistent whatever the
 * drafts settle on.
 */
