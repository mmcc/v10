/**
 * LOC (Low Overhead Media Container, draft-ietf-moq-loc-04) frame extraction.
 *
 * With LOC packaging each MOQT object carries exactly one encoded frame: the object payload is the "internal data" of a
 * WebCodecs `EncodedVideoChunk`/`EncodedAudioChunk`, and the object properties carry frame metadata (capture timestamp,
 * timescale, decoder config).
 *
 * DOM-free and wire-free: properties come in as generic key/value pairs (structurally identical to `network/moqt`'s
 * `KeyValuePair` — the layering keeps `media/` independent of `network/`, and structural typing lets the playback layer
...
/**
 * LOC property IDs from the MOQ Properties registry (loc-04 §6.1), which settled the pre-IANA collisions of loc-02/03:
 * 0x08 TIMESCALE (Track, Object), 0x09 VIDEO_FRAME_MARKING (Object), 0x0C AUDIO_LEVEL (Object), 0x0D VIDEO_CONFIG
...
/**
 * The TIMESCALE (0x08) declared at _track_ scope, from a SUBSCRIBE_OK's Track Properties.
 *
 * Track Properties and Object Properties share one registry, so this is the same code point `parseLocProperties` reads
 * per object — the scope is the difference. Reading it is not optional bookkeeping: a publisher states the units once
 * per track, and moq-dev relays from 0.14.6 (upstream `08224275`) stopped repeating TIMESCALE on every object because
 * restating a per-track constant costs bytes per frame. So for those relays this is the _only_ declaration on the wire,
 * and a reader without it silently falls back to microseconds — correct only while every track happens to be
 * microsecond-scaled, and wrong by a factor of 1000 on the first track that is not.
 *
 * Returns `undefined` when no usable declaration is present, which includes a zero: a timescale of zero would make
 * every conversion a division by zero, and treating it as absent lets the documented fallback chain handle it.
 */
...
export interface LocFrame {
  ...
  timestampUs: number;
  /**
   * Key frame flag. MSF maps one GOP per MOQT group (msf-01 §4.1), so the group's first object is the random-access
   * point.
   */
  isKey: boolean;
...
/**
 * Convert a LOC timestamp to WebCodecs microseconds. Without any timescale the value is already microseconds (loc-04
 * §2.3.1.1).
 */
...
/**
 * Extract the decodable frame from a LOC-packaged object. Returns `null` for objects with no payload (status markers)
