 ...
 *
 * - `'pull'` — the player computes what it needs and fetches it (segmented HTTP delivery: HLS/DASH). The default when
 *   absent.
 * - `'push'` — the player subscribes and media objects arrive as the publisher produces them (MoQ). No segment list ever
 *   exists.
 */
...
/**
 * A push-delivered live track: full selection metadata (codecs, bandwidth, dimensions, language) but no segment list —
 * media arrives via subscription (e.g. MoQ), so a segment list never exists and the track never becomes a
 * `ResolvedTrack`.
 *
 * Structurally a `PartiallyResolved<T>` narrowed to `deliveryMode: 'push'`, which makes it storable in switching sets
 * and consumable by track selection as-is — selection reads only metadata. The difference is semantic, carried by the
 * discriminant: a partially resolved pull track is _waiting_ to be resolved (media playlist not fetched yet), while a
 * live track is terminal — there is nothing further to resolve from a manifest. Use `isLiveTrack` to tell them apart.
 */
...
/**
 * Check if a track is push-delivered (live, no segment list — see `LiveOf`). Works for all track types with overloaded
 * signatures for type narrowing.
 */
