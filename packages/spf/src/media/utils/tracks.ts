...
/**
 * Get the tracks of the given type from EVERY switching set of the matching selection set — the all-sets counterpart of
 * {@link getTracksByType}. Sibling switching sets hold _different content_ (a MoQ publisher's screen share beside its
 * camera), so this is for callers that must see or address all content items; the default candidate pool stays
 * first-set-only. HLS presentations have one set per type, where the two are identical.
 */
 ...
 *
 * Searches EVERY switching set of the matching selection set — an id-addressed lookup must be able to resolve an
 * explicit cross-set selection (see {@link getTracksAcrossSwitchingSets}). The returned track may be partially resolved
 * (URL only) or fully resolved (with segments) — callers narrow as needed.
 */
 ...
 *
 * Searches every switching set — an id-addressed lookup must resolve tracks in sibling sets (distinct content items,
 * e.g. a MoQ screen share), same as {@link findTrack}. Track ids are unique within a presentation per the HLS spec; the
 * first match wins.
 */
