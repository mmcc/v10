 ...
 *
 * Pulls LOC frames from a frame source (the track-subscriber jitter buffer), decodes ahead of the playout clock, and
 * presents decoded `VideoFrame`s against the **master clock** by timestamp with a hold-early / drop-late policy.
 * Presentation runs on `requestAnimationFrame` (vsync-aligned) when available, falling back to the decode interval;
 * decode always runs on the interval so the jitter buffer keeps draining while rAF is throttled (hidden tab). Not
 * `requestVideoFrameCallback`: rVFC only exists on `HTMLVideoElement`, and there is none here.
 *
 * The clock is injected (`getClockTimeUs`): the audio renderer owns the master clock when audio plays; without one the
 * renderer self-clocks — anchored on its first decoded frame or on `getTargetClockUs()` (the delivery edge), advancing
 * by wall time × `getPlaybackRate()`, and **continuously slewed back onto the edge**. Either way the backlog behind the
 * anchor is decoded in order and dropped late, so joining at the edge fast-forwards through it instead of skipping its
 * keyframe.
 *
 * Track switches are keyframe-gated: `setTrack` swaps the frame source and decoder config, and decode resumes only from
 * the next keyframe-led frame (make-before-break handoffs guarantee one is already buffered).
 */
...
export interface VideoRendererContext {
  ...
  /**
   * Timestamp of the last frame presented **from the current track**, and `undefined` before this renderer has
   * presented one. Cleared by `setTrack` — see the reducer — because it is read as a playout position and not as a
...
export interface VideoRendererConfig {
  ...
  /**
   * Fraction of real time the self-clock may spend correcting itself onto `getTargetClockUs()`. Default 0.05 → at most
   * 50ms of correction per second. Staying well below the playout rate is what keeps the clock monotonically forward
   * while it is being pulled backwards, so a supplied value is clamped into `[0, 0.9]` and a non-finite one takes the
   * default.
   */
...
export interface CreateVideoRendererOptions extends VideoRendererConfig {
  ...
  /**
   * **Where the self-clock should be right now**: the delivery edge, one target latency back. Consulted on _every_
   * clock read, not only at join — the self-clock anchors on it, then slews back onto it (bounded by `clockSlewRate`)
   * for as long as it self-clocks. Frames behind the anchor are still decoded in order from their keyframe and then
   * dropped late, so a backlog fast-forwards rather than replaying. Ignored while a master clock is supplied: that
   * clock carries the anchor itself.
   */
...
export interface VideoRendererActor extends Pick<TransitionActor<VideoRendererContext, RendererMessage>, 'snapshot'> {
  /**
   * Point the renderer at a (new) frame source. Decode restarts at the next keyframe with the given decoder config.
   * `null` source stops rendering (status `'idle'`).
   *
   * `lastPresentedTimestampUs` clears with it: it is the published playout position, and the frame it named belonged to
   * the departed track. The cumulative counters do not.
   */
  setTrack(source: VideoFrameSource | null, config: VideoDecoderConfig | null): void;
  /**
   * Current playout clock in media microseconds — the injected master clock when there is one, otherwise the
   * self-clock; `undefined` before either has a value. Reading it is side-effect-free from the caller's point of view:
   * the self-clock's slew is budgeted per unit of _real_ time, so extra reads split the same correction into smaller
   * steps rather than applying it more often.
   */
  getClockTimeUs(): number | undefined;
  destroy(): void;
}
...
/**
 * Media-time jump beyond which two readings are on different timelines (see `TIMELINE_DISCONTINUITY_US`). Applied twice
 * here: buffered frames this far ahead of the self-clock are a timeline reset (latency catch-up skipped groups) rather
 * than frames to wait out in real time, and a master clock this far behind every held frame is a foreign timeline to
 * stand down from rather than a position to hold for.
 */
const DISCONTINUITY_THRESHOLD_US = TIMELINE_DISCONTINUITY_US;
/**
 * How long the foreign-master geometry must persist before the renderer stands down to its self-clock. A same-timeline
 * catch-up skip shows the same gap transiently: it jumps both jitter buffers, but the master follows only once the
 * audio renderer's schedule horizon (~4 × its 50ms margin) reopens and it re-anchors onto the skipped-to timeline.
 * 250ms outlives that window; a genuine timeline step persists indefinitely and only pays this as a slightly longer
...
/**
 * Upper bound on `clockSlewRate`. The slew subtracts up to `slewRate` of real time from a clock advancing at
 * `getPlaybackRate()` of it, so the clock only stays monotonically forward while the rate is below the slowest playout
 * rate it will ever be read at. 0.9 leaves headroom under the latency controller's `1 - rateNudge` (0.95 by default) —
 * the one property the bounded slew exists to guarantee.
 */
const MAX_CLOCK_SLEW_RATE = 0.9;

/**
 * Clamped rather than validated, and a broken value takes the default rather than 0: this actor is handed a slew rate
 * from a latency config several layers up, and the failures are the class nothing reports. At or above 1 the correction
 * outruns playback and the clock stalls or walks backwards; negative inverts the correction's sign, so the clock is
 * driven _away_ from the edge and re-anchored there on every read, diverging without bound; `NaN` poisons the anchor
 * permanently. A slower correction is a worse clock, but a stalled, diverging or frozen one is not a clock.
 */
...
export function createVideoRendererActor(options: CreateVideoRendererOptions): VideoRendererActor {
  ...
  /**
   * Wall time of the last edge-tracking evaluation. Separate from `selfAnchor.wallMs` (which only moves when the anchor
   * is rewritten) so the slew budget accrues over the interval actually elapsed since the last correction _opportunity_
   * — otherwise a long stretch inside the tolerance band banks budget and the next correction lands as a jump.
   */
  let lastSlewWallMs: number | null = null;
  /**
   * Wall time the foreign-master geometry was first observed, `undefined` while the master is adopted (or absent). See
   * `FOREIGN_MASTER_CONFIRM_MS`.
   */
  ...
  /**
   * Where the self-clock should start from `fallbackUs`: the target clock when it sits ahead (the delivery edge is
   * further on than this frame), the frame itself otherwise. Never moves the clock backwards — a buffer shallower than
   * the target latency is already at the edge.
   */
  ...
  /**
   * **Track the delivery edge.** A self-clock anchored once and extrapolated from `performance.now()` forever cannot
   * stay right: publisher clock and consumer clock drift, an anchor placed mid join-burst is placed against an edge
   * that had not finished arriving, and every hold (pause, rate 0) ratchets the offset up permanently. So the clock
   * re-reads `getTargetClockUs()` — the live edge, one target latency back — on every read and steers back onto it.
   *
   * The correction is a slew, not a jump: at most `slewRate` × the real time elapsed since the last evaluation, and
   * only outside a `slewToleranceUs` band (the edge advances in frame-sized steps, so a tighter band would chase
   * quantization noise). That bound is what makes this safe rather than merely correct — with `slewRate` < 1 the clock
   * stays monotonically forward even while being pulled backwards, and a mis-placed join anchor self-corrects invisibly
   * instead of being held for the life of the stream.
   *
   * **Versus the latency controller's rate nudge** (`syncLatency`, ±5%, which reaches this clock through
   * `getPlaybackRate`): both are negative feedback on the _same_ error — the controller measures edge-to-playout
   * against the same resolved target this reads — so they always pull the same direction and cannot fight. They divide
   * by scale: the slew works continuously and owns fine error, including inside the controller's deadband where nothing
   * has engaged the controller; the controller owns error coarse enough to leave the deadband, and holds its correction
   * back down to its own reclaim band rather than to the deadband edge — so both are active over part of the deadband,
   * in the same direction. They compound to at most `slewRate + rateNudge` of real time, still bounded and still
   * sub-perceptual on a video-only self-clock. Error beyond `DISCONTINUITY_THRESHOLD_US` is neither's job — that is a
