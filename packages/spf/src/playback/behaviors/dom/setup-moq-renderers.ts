/**
 * **Wire subscriber jitter buffers into the WebCodecs renderers.** Four behaviors:
 *
 * - `setupAudioRenderer` — owns `context.audioRendererActor` (created when `context.audioContext` appears) and points it
 *   at the active audio subscriber. The audio renderer owns the **master clock**.
 * - `setupVideoRenderer` — owns `context.videoRendererActor` (created when `context.renderSurface` appears), points it at
 *   the active video subscriber, and slaves its presentation to the audio renderer's clock when one exists (falling
 *   back to the renderer's self-clock for video-only playback).
 * - `trackPlayoutTime` — owns `state.currentTime`: a playout-cadence interval publishes whichever clock is running as
 *   seconds (the MoQ engine has no HTMLMediaElement to read time from). It is its own behavior rather than a second job
 *   of the audio side because it must run whenever _either_ renderer exists: gated on the AudioContext it would go
 *   silent exactly in the video-only case it exists to cover, and `syncLatency` reads `currentTime` as its setpoint, so
 *   a starved clock is a stopped controller.
 * - `trackPlayoutHealth` — owns `state.framesDropped` and `state.audioUnderruns`: the renderers' quality-cost counters,
 *   which lived inside the actors and were readable from nowhere. Separate from the clock above because it has a
 *   different consumer and a different cadence, and always composed because instrumentation that only exists when
 *   adaptation is on cannot measure whether adaptation helped.
 *
 * The renderers apply `state.playoutRate` (latency-controller nudges) through the `getPlaybackRate` seam — gated to 0
 * for video while `state.paused` is set, so video-only playback actually freezes on pause — and both re-point on
 * subscriber-actor swaps, which is the moment a make-before-break handoff completes; the renderer's keyframe gate
 * handles the decoder reconfiguration.
 *
 * Both also aim their clocks at the **delivery edge**: with `latency.joinAtEdge` set they hand each renderer the live
 * edge of its own jitter buffer (newest buffered − target latency). The two legs use it differently, which is why the
 * option is named differently on each — audio consults it once per join (`getJoinAnchorUs`; audio cannot fast-forward a
 * backlog without pitch artifacts, so it drops it), while video consults it continuously (`getTargetClockUs`) and slews
 * its self-clock onto it. The controller in `sync-latency` steers latency at coarse scale once playout is running;
 * where the clocks _are_ lives here, because the renderers own them.
 */
...
export interface MoqRendererState {
  ...
  /**
   * Adapter-written pause flag; `undefined` means playing. Gates the renderers' playout rate to 0 — the video
   * self-clock re-anchors on rate changes, so rate 0 holds presentation exactly and resumes from the hold point (there
   * is no audio master clock to freeze in video-only playback).
   */
  paused?: boolean;
  currentTime?: number;
  /**
   * Which renderer `currentTime` was last sampled from, and `undefined` when neither is producing a position. Owned by
   * `trackPlayoutTime` and read by `syncLatency`, which can only call `newest buffered − currentTime` a latency if it
   * measures the edge of the track that position came from, and only while that position is still live — `currentTime`
   * holds its last value when the clock stops, so this is what says it stopped.
   */
...
/**
 * The live edge of `subscriberSignal`'s jitter buffer, `targetLatency` back: where the renderer's clock should be.
 * `undefined` disables edge tracking — the knob is off, there is no subscriber, or nothing has arrived yet.
 */
...
/**
 * The audio join anchor: the audio buffer's live edge, but never behind a video clock that is already running.
 *
 * Audio subscriptions can start late — an autoplay deferral unlocks on first gesture, a sustained pause releases and
 * rejoins — and their edge is computed from a buffer that has only just started filling. Anchoring there can land
 * behind the video self-clock, and the video renderer only re-anchors on _forward_ discontinuities, so it would hold on
 * its last frame until the newly-installed master clock caught up to it. Clamping forward is the same "never move the
 * clock backwards" rule the video renderer applies to its own anchor.
 *
 * **An absent edge takes the video clock too.** `makeEdgeTargetUs` yields `undefined` while the subscriber's snapshot
 * has no `newestTimestampUs`, and the renderer consumes this anchor in exactly one place: as the threshold below which
 * it discards buffered audio unheard. An absent threshold is therefore not a neutral default but _no trim at all_ — the
 * schedule starts at the oldest frame the renderer is holding.
 *
 * Those two are wired separately. The anchor peeks the subscriber signal when the renderer asks, while the frame source
 * is pushed in by the `setTrack` effect below and trimmed on the renderer's own tick. So the renderer can be asked for
 * a threshold while the subscriber it reads reports nothing and the buffer it would trim already holds frames — audio
 * behind the running video clock, played instead of dropped. Deferring to the video clock keeps this function's
 * invariant applying in the one case that could not clamp: audio with no edge of its own.
 *
 * With no video clock — an audio-only broadcast — the anchor stays absent and the oldest-buffered default stands, which
 * is right: nothing else is steering, and there is no second timeline to agree with.
 *
 * **Unless the video clock is a whole timeline step ahead of the audio delivery edge.** The forward clamp assumes the
 * two tracks share a timeline; a publisher that re-anchors the audio timeline on a capture source switch breaks that,
 * and the video clock — still on the departed timeline — then sits past everything the new timeline will deliver for a
 * long time (a backward re-anchor: forever). Clamping onto it would discard every arriving audio frame unheard. A video
 * clock more than `TIMELINE_DISCONTINUITY_US` past the audio _edge_ (the newest frame delivered, so the furthest any
 * same-timeline clock could plausibly read) is therefore ignored, and the audio edge anchors alone.
 */
...
/**
 * **Publish the playout position as `state.currentTime`** (media seconds), sampled from the audio master clock when
 * audio is scheduled and from the video renderer's last presented frame otherwise — **and publish which of the two it
 * was** as `state.playoutClockOwner`.
 *
 * The owner is not bookkeeping. `syncLatency` measures `newest buffered − currentTime`, which is only a latency if the
 * edge and the position come from the same track, and nothing outside this interval can tell which track the position
 * came from: the audio renderer's `getClockTimeUs()` is undefined until it has actually scheduled a buffer, so an audio
 * subscriber, an AudioContext, even a filling audio jitter buffer can all be present while the position on offer is
 * still video's last presented frame. A controller inferring the owner from those instead measures across two timebases
 * for the whole of that window. Owner and position are therefore written together, owner first, from one sample.
 *
 * When neither clock offers a position the owner is cleared and the position is not. `currentTime` has two other
 * readers that treat `undefined` as a statement rather than an absence — the media element's `currentTime`, and the
 * handoff promotion gate — so the last known position stands, and the owner is what says it is no longer live.
 *
 * Ungated on purpose. Its two consumers — the media-element facade's readiness derivation and `syncLatency`'s setpoint
 * — both need a value in exactly the configurations a gate would exclude (no AudioContext, audio deferred behind an
 * autoplay unlock, audio released by a sustained pause), and sampling two absent renderers costs one no-op interval.
 *
 * @example
...
/**
 * **Publish the renderers' quality-cost counters as engine state**: `framesDropped` (video presented-late discards) and
 * `audioUnderruns` (times the audio schedule ran dry).
 *
 * Both already exist inside the renderer actors and neither was readable from outside them. They are the _cost_ half of
 * any latency decision — "the target came down" is only half an answer without "and nothing started dropping" — so they
 * are published whether or not adaptation is running: an A/B whose control arm is uninstrumented cannot be read.
 * `adaptLatencyTarget` is the other reader, which is why this is a separate always-on behavior rather than part of it.
 *
 * Interval-sampled rather than effect-driven on purpose: `framesDropped` lives in the same actor snapshot as
 * `lastPresentedTimestampUs`, so an effect over it would re-run on every presented frame to copy a number that changes
 * far more rarely.
 *
 * @example
