/**
 * **Suspend media delivery when a pause stops being transient.** Owns `state.mediaSuspended`: set once `state.paused`
 * has held continuously for the pause hold window, cleared the moment playback resumes (and on destroy). The subscribe
 * behaviors read it as a gate — suspension releases the media track subscriptions while the session and catalog
 * subscription stay open, so a paused player stops downloading live media it would only discard.
 *
 *     'playing' ↔ 'paused'
 *
 * The hold window defaults to _target latency + catch-up threshold_ — the depth at which `syncLatency` starts
 * discarding the paused buffer via group skips. Shorter pauses (scrubbing, quick toggles) keep the buffer intact and
 * resume from the hold point; anything longer was going to resume at the live edge anyway, so releasing the
 * subscriptions changes nothing visible. The window derives from the same target `syncLatency` is holding —
 * `state.targetLatency`, else `state.adaptiveTargetLatency` — falling back to the shared `latency` config. It reads the
 * adaptive proposal rather than the raw consumer input because the window's whole meaning is "the depth at which the
 * controller starts discarding the paused buffer", and with adaptation running that depth is the adaptive target;
 * sizing it off an input the controller is not using would put the release at the wrong moment in exactly the
 * configuration the knob exists for. The per-track catalog `targetLatency` is still not consulted — the selected track
 * can change mid-pause, and the shared default keeps the window stable. `config.pauseHoldSeconds` overrides the
 * derivation outright.
 *
 * Because the adapter parks `paused: true` before the first `play()`, the hold window also bounds `preload: 'auto'` on
 * a never-played source: media is joined, buffered, and then released until playback actually starts. `undefined` means
 * playing (engine-only drivers never pause by default, matching the renderers' reading of the flag), so drivers that
 * never write `paused` never suspend.
 *
 * Sole writer of `mediaSuspended`. Transition-driven: the timer starts on pause entry, and the entry-returned cleanup
 * (clear timer + clear slot) fires on resume and on destroy alike.
 */
...
export interface SuspendMediaWhilePausedConfig {
  /**
   * Continuous pause duration, in seconds, before media delivery suspends. Defaults to target latency +
   * `latency.catchUpThreshold`.
   */
