/**
 * **Probe WebCodecs encoder support for each captured track and pick the active encodings.** Three independent probes —
 * camera, screen, mic — each running while its own `state.*Tracks` fact exists: runs `VideoEncoder.isConfigSupported` /
 * `AudioEncoder.isConfigSupported` over a candidate ladder derived from `config.{camera,screen,audio}` plus the
 * captured track settings, merges the result into `state.encoderSupport` (partitioned by kind — see below), and
 * resolves `state.activeEncodings` through the `config.selectEncoderConfig` strategy seam (default: first supported per
 * kind), applying only that probe's own kind from the strategy's result.
 *
 * Default ladder: H.264 constrained-baseline (`avc1.42E01F`, `avc` bitstream format so the avcC extradata exists to
 * publish out-of-band — see `videoCandidates`) with a VP8 fallback, at the track resolution/framerate; Opus at the
 * track sample rate with a 48 kHz fallback. A `config.{camera,screen,audio}.codec` prepends itself to the ladder rather
 * than replacing it — for audio the preferred codec probes at every ladder sample rate before Opus does (codec
 * preference outranks rate preference), so `audio: { codec: 'mp4a.40.2' }` publishes AAC where the browser can encode
 * it and falls back to Opus where it can't. The screen ladder defaults to a lower framerate than the camera's (static
 * degrade-screen-first tuning, per the multi-source design record's "Encoder budget" decision — no dynamic policy in
 * v1).
 *
 * Each probe is a single-positive-state reactor mirroring the acquire behaviors: the probe runs in the positive state's
 * `effects:` so a tracks-identity change (re-acquire) re-probes through the cleanup, and an in-flight probe that
 * resolves after the tracks changed discards via the per-run `stale` flag.
 *
 * `state.encoderSupport` / `state.activeEncodings` are multi-writer slots partitioned by kind — the same pattern
 * `publishError` already uses: each probe is the sole writer of its own key and never touches the others', so camera
 * support resolving never clobbers screen's (or vice versa) and either can come and go independently through a session.
 * The last kind leaving restores the absent state rather than an empty object — see `withoutKind`.
 *
 * Co-writer of `state.publishError`, for one condition only: a kind whose candidate ladder probed entirely unsupported.
 * A `selectEncoderConfig` omitting a kind is a policy veto, not a failure. Such a verdict stands until the kind
 * recovers or its source is released — including across another writer taking the slot and later clearing it — see the
...
/**
 * Strategy seam resolving probed support into the active encodings — e.g. to prefer a codec, cap resolution, or veto a
 * kind entirely. Called with the full merged support snapshot; each probe applies only its own kind from the result.
 */
...
export interface AudioEncodeTuning {
  bitrate?: number;
  /**
   * Preferred WebCodecs audio codec string (e.g. `'mp4a.40.2'` for AAC-LC). Prepended to the Opus default rather than
   * replacing it, so an unsupported preference degrades to Opus instead of failing the kind.
   */
  codec?: string;
}
...
/**
 * Drop one kind from a per-kind facts object, collapsing an emptied result back to `undefined`. `{}` is truthy and
 * downstream gates on presence, not on content: `deriveCatalog` would sit in 'publishing-catalog' and publish a
 * track-less catalog once the last source was released, instead of dropping to idle.
 */
...
function probeEncoderSupportSetup({
  ...
  /**
   * The encode verdicts THIS behavior instance wrote, per kind, in authoring order. `publishError` is one slot with
   * several writers, so the arbitration rule is:
   *
   * - **Any writer may take the slot** — last write wins, including over ours. Membership here is what we believe, not
   *   what is displayed.
   * - **We retract only the object we put there** (identity-compared), so a capture or transport failure written after
   *   ours survives our recovery; the slot passes to our next outstanding kind, if any.
   * - **We retake the slot whenever it falls empty** while a kind is still un-encodable (`reassertOutstandingError`).
   *   Other writers retract their own errors on their own schedule — a fresh acquisition clears a stale capture failure
   *   — and an unsupported encoder ladder is not fixed by that; without this the UI would show nothing for a broken
   *   source.
   */
