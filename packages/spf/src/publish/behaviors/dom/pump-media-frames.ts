/**
 * **Pump capture-track frames into the encoder actors.** Three independent read loops — camera, screen, mic — each
 * spawned while its own stream/actor pair (`context.{cameraStream,screenStream,micStream}` + the matching encoder
 * actor) is live, using one `MediaStreamTrackProcessor` per loop and dispatching every frame as an `encode` message.
 * The two video loops force `keyFrame: true` on the group cadence (`config.groupDurationSec`, default 2 s each, tracked
 * independently per kind so a screen share starting mid-session starts its own group sequence rather than inheriting
 * the camera's), computed from frame timestamps rather than wall clock so paused/hidden capture doesn't skew GoP
 * boundaries. Loops abort on state exit, stream change, or teardown.
 *
 * Frame ownership: the encoder actor takes ownership at `send()` and closes on every path. Frames the loop cannot hand
 * off — teardown races, an actor already closed/destroyed — are closed here; no frame outlives its dispatch decision.
 *
 * Pure-dispatcher reactor per the setup-actor convention: it reads the actor slots `setupEncoderActors` owns and sends
 * messages — it never creates actors. Co-writer of `state.publishError` (missing `MediaStreamTrackProcessor` platform
 * support only).
 */
...
/**
 * Timestamp-driven keyframe cadence: fires on the first frame and whenever `groupDurationUs` of media time has elapsed
 * since the last forced key.
 */
...
/**
 * Read frames until the track ends or the returned cleanup cancels the reader. Every frame is either handed to the
 * actor (which closes it) or closed here.
 */
