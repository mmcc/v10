/**
 * **Own the publish transport session.** While an `endpoint` is set, `publishActivated` is true, and any capture
 * pipeline (camera, screen, or — under explicit `micActive` intent only — microphone) is `'active'`, connects through
 * `config.connectTransport` (default: a real `WebTransport`), creates the publish-session actor, publishes it on
 * `context.publishSessionActor`, and mirrors the actor's lifecycle into `state.sessionStatus` (`connecting → ready →
 * live`, `draining` on GOAWAY, `error` on failure). When the gate collapses — unpublish, capture release, or teardown —
 * the actor is destroyed, which FINs every live subscription's stream and retracts the namespace announce, drains those
 * writes briefly, and closes the transport; `sessionStatus` settles on `'closed'` (a prior `'error'` is preserved).
 *
 * DOM-free: the session drives a structural `MoqtTransport`, and the `WebTransport` default comes from the WebWorker
 * lib (the same shape the playback `moq-session` actor uses), so only tests and alternative hosts need to inject
 * `connectTransport`.
 *
 * Sole writer of `state.sessionStatus` and `context.publishSessionActor`; co-writer of `state.publishError`
 ...
 *
 * `ready` — transport + setup complete, waiting for the peer to solicit the namespace. `live` — the namespace is
 * announced on an accepted solicitation, so the peer can route subscriptions to us (ingest is pull-through: data still
 * flows only per inbound subscription). `draining` — orderly shutdown (unpublish or GOAWAY).
 */
export type PublishSessionStatus = 'idle' | 'connecting' | 'ready' | 'live' | 'draining' | 'closed' | 'error';

/**
 * Structural mirror of `behaviors/dom/acquire-capture-source.ts`'s `CaptureStatus` (DOM-bound, so not importable here)
 * — keep identical.
 */
export type CaptureStatusFacts = 'idle' | 'acquiring' | 'active' | 'denied' | 'ended';

/** Structural mirror of `behaviors/dom/acquire-capture-source.ts`'s `PublishErrorFacts` — keep identical. */
