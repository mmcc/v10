/**
 * Shared capture + playback harness for the cross-engine suites (`publish/engines/moq/tests/cross-engine*.test.ts`):
 * the synthetic capture device (animated canvas + oscillator audio) and a real playback engine joined to the relay
 * hub's broadcast. Per-suite `getUserMedia`/`getDisplayMedia` stubbing stays in each suite — which pipeline gets which
 * stream (and with what injected skew) is the scenario under test.
 *
 * Helpers register their cleanup on the caller's `disposals` array, the suites' shared teardown idiom (`afterEach`
 * drains it).
 */
