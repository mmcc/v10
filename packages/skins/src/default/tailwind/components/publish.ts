...
/**
 * Publisher elements signal disabled via `data-disabled` (custom elements can't rely on the native `disabled`
 * attribute); mirrors the `disabled:` treatment in `button.base`.
 */
...
/**
 * Publish button — the primary call to action. White pill by default ("Go live"), solid red while live ("Stop"). Pairs
 * with `button.base`.
 */
...
/**
 * Status row — overlay capsule for badge + timer + connection indicator. Positioned top-left, mirroring the controls
 * inset.
 *
 * The row carries its own ground rather than relying on text shadows: it sits over an arbitrary camera frame, and a
 * bright one leaves bare white text (the timer especially) unreadable. It borrows the surface border/shadow tokens but
 * takes a dark fill instead of the glass one — glass tracks the frame behind it, which is what fails over bright
 * video.
 */
...
/**
 * Publish badge — status text with a leading dot (mirrors the live button's dot in `button.live`). The status capsule
 * is already its ground, so the badge drops the `badge` fill and only takes a pill back when a state needs to stand on
 * its own: solid red live, tinted red error. Pairs with `badge`.
 */
...
export const publishTimer = cn(
  'hidden whitespace-nowrap text-(length:--media-font-size-small) tabular-nums',
