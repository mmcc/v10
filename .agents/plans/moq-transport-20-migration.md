...
**Phase 5 status (2026-09-03, `mmcc/moq-publisher`):** the branch merged the
cutover. Landed with the merge: inbound REQUEST_UPDATE consumes its own Request
ID in `publish-session.ts` (reuse is session-fatal; `onRequestUpdate` reports
