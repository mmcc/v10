...
- **Scope (v1)**: single video rendition + Opus audio; camera + mic + screen share (exclusive — a share replaces the camera; simultaneous sources are proposed in [moq-publisher-multi-source.md](./moq-publisher-multi-source.md)); Chromium-only (`MediaStreamTrackProcessor`, WebTransport); no FETCH serving, datagrams, subscribe filters, catalog deltas, reconnect, or simulcast — the track/actor model leaves simulcast additive (`config.video` as an array is the seam).
...
- **Nearly no new wire codecs.** The parent MoQ branch's draft-19 control-message codec was already symmetric; the publish direction added sibling modules — `network/moqt/subgroup-writer.ts` (the write complement of the object-stream reader) and `media/moq/{loc-packaging,build-catalog}.ts` (the encode complements of `loc.ts` / `parse-catalog.ts`, round-trip-tested against them) — and the announce-and-serve revision added two encoders (`encodeNamespace` / `encodeNamespaceDone`, whose decoders already existed) plus the `TRACK_PROPERTY.TIMESCALE` constant to the codec module.
- **Flow choice.** Announce-and-serve (revised 2026-08-07; the original publisher-initiated PUBLISH flow died with moq-relay 0.14.7, which rejects every PUBLISH): the relay solicits authorized namespaces with SUBSCRIBE_NAMESPACE right after SETUP, the publisher answers REQUEST_OK and announces via a NAMESPACE entry on that stream, and inbound SUBSCRIBEs on registered tracks are answered with SUBSCRIBE_OK (alias = request ID, microsecond TIMESCALE declared) — data flows per subscription, video joining at the next keyframe and the catalog replaying its latest frame on bind. A track ends by a bare FIN of its subscription streams (a trailing PUBLISH_DONE makes the relay abort the track), and the client never sends GOAWAY (session-fatal on moq-lite-rs). See the feature record's ingest-model decision for the full peer-verified rules.
