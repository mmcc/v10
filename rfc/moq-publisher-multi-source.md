...
- **Additive capture sources → separate tracks in one session (recommended)** — camera, screen, and microphone become independent capture pipelines feeding independent encoders and track publishers under one namespace and catalog. The moqt layer already supports this: tracks register independently on the session's serve registry (each subscription carries its own alias and per-track group/backpressure accounting), ending one track FINs only its own subscriptions, and `deriveCatalog` re-sends a full independent catalog whenever the active encodings change — a track appearing or disappearing mid-session already round-trips. The v1 RFC reserved exactly this seam ("`config.video` as an array").
