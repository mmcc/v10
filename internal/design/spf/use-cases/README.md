...
- [`moq-live-publish`](./moq-live-publish.md) *(sketched)* — browser live ingest to a MoQ relay: the publish direction (capture → WebCodecs encode → MOQT) as its own composition family (`createMoqPublishEngine`, `publish/` peer of `playback/`) rather than a variant of a playback engine. Constituent capability: [`moq-publish`](../features/moq-publish.md).
